...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    try {
      ...
      // Una sesión anterior puede haber caducado o ya no tener perfil. El portal
      // debe seguir disponible para que el usuario vuelva a iniciar sesión.
    ...
    // La pantalla de inicio de sesión no depende de la carga inicial de datos.
    // De este modo, un fallo transitorio no bloquea los accesos del portal.
    console.warn('La carga inicial no se completó; puedes iniciar sesión de nuevo.', err);
  }
  ...
  empLoginForm.addEventListener('submit', async (e) => {
    ...
    const res = await window.authService.loginEmployee(empLoginEmail.value, empLoginPassword.value);
    if (res.success) {
  ...
  btnEmployeeLogout.addEventListener('click', async () => {
  ...
  managerLoginForm.addEventListener('submit', async (e) => {
    ...
    const res = await window.authService.login('manager', mgrUsername.value, mgrPassword.value);
    if (res.success) {
      await loadAllExpenses();
  ...
  btnManagerLogout.addEventListener('click', async () => {
    await window.authService.logout();
  ...
  financeLoginForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    finLoginError.classList.add('hidden');
    const res = await window.authService.login('finance', finUsername.value, finPassword.value);
    if (res.success) {
      await loadAllExpenses();
  ...
  btnFinanceLogout.addEventListener('click', async () => {
    await window.authService.logout();
  ...
  adminLoginForm.addEventListener('submit', async (e) => {
    ...
    const res = await window.authService.login('admin', admUsername.value, admPassword.value);
    if (res.success) {
      await loadAllExpenses();
      ...
      areaManagersTableBody.innerHTML = `
        <tr>
          <td colspan="4" class="px-4 py-8 text-center text-slate-500">
            No hay perfiles de empleado visibles.
          </td>
        </tr>
      `;
      return;
  ...
  // ----------------------------------------------------
  // TABLA: DIRECTORIO DE CUENTAS (ADMIN, SOLO LECTURA)
  // ----------------------------------------------------
  function renderUsersTable() {
    ...
    usersTableBody.innerHTML = users.map(user => {
      ...
      return `
        <tr class="hover:bg-slate-50 transition">
          <td class="px-4 py-3.5"><span class="font-mono font-bold text-slate-900">@${escapeHtml(user.username)}</span></td>
          <td class="px-4 py-3.5"><div class="font-black text-slate-900">${escapeHtml(user.name)}</div><div class="text-[11px] text-slate-500 font-mono">${escapeHtml(user.email || '')}</div></td>
          <td class="px-4 py-3.5"><span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-black ${roleBadgeColor}">${escapeHtml(user.roleLabel || user.role)}</span></td>
          <td class="px-4 py-3.5"><span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-black ${user.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-500'}">${user.status === 'ACTIVE' ? 'Activo' : 'Inactivo'}</span></td>
        </tr>`;
    }).join('');
  }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const currentEmp = window.authService.getCurrentEmployee();
    if (!currentEmp) {
      showToast('Inicia sesión como Area Manager para registrar un gasto.', 'error');
      return;
    }

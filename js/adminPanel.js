...
(() => {
  ...
  function renderModal(users, user = null) {
    ...
    root.innerHTML = '<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80"><form id="adminUserForm" class="bg-white rounded-3xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-6 space-y-4 shadow-2xl">'
      ...
      + '<label class="text-xs font-black block">Primer jefe<select id="adminBoss" class="mt-1 w-full px-3 py-2 border rounded-xl">' + managerOptions(users, user?.direct_boss_id) + '</select></label>'
      + '<label class="text-xs font-black block">Segundo jefe<select id="adminSecondBoss" class="mt-1 w-full px-3 py-2 border rounded-xl">' + managerOptions(users, user?.secondary_boss_id) + '</select></label>'
    ...
    document.getElementById('adminUserForm').onsubmit = async (event) => {
      ...
      const payload = { full_name: document.getElementById('adminFullName').value.trim(), username: document.getElementById('adminUsername').value.trim(), role: document.getElementById('adminRole').value, region: document.getElementById('adminRegion').value.trim(), direct_boss_id: document.getElementById('adminBoss').value || null, secondary_boss_id: document.getElementById('adminSecondBoss').value || null };
      try {

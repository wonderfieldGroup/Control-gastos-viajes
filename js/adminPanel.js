...
(() => {
  ...
  const strongPassword = (value) => value.length >= 14 && /[a-z]/.test(value) && /[A-Z]/.test(value) && /[0-9]/.test(value) && /[^A-Za-z0-9]/.test(value);
  ...
  function renderModal(users, user = null) {
    ...
    root.innerHTML = '<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80"><form id="adminUserForm" class="bg-white rounded-3xl max-w-lg w-full p-6 space-y-4 shadow-2xl">'
      ...
      + (!editing ? '<label class="text-xs font-black block">Contraseña temporal<input id="adminTempPassword" type="password" required autocomplete="new-password" class="mt-1 w-full px-3 py-2 border rounded-xl" placeholder="14+ caracteres, mayúscula, minúscula, número y símbolo"></label>' : '<label class="flex gap-2 items-center text-xs font-bold"><input id="adminActive" type="checkbox"' + (user.active ? ' checked' : '') + '> Cuenta activa</label>')
      + '<div class="flex justify-end gap-2 pt-2"><button type="button" id="cancelAdminUser" class="px-4 py-2 text-xs font-bold">Cancelar</button><button class="px-5 py-2 rounded-full text-xs font-black text-white bg-[#433364]">' + (editing ? 'Guardar cambios' : 'Crear cuenta') + '</button></div></form></div>';
    ...
    document.getElementById('adminUserForm').onsubmit = async (event) => {
      ...
      try {
        ...
        else { const password = document.getElementById('adminTempPassword').value; if (!strongPassword(password)) throw new Error('La contraseña temporal no cumple la política de seguridad.'); payload.email = document.getElementById('adminEmail').value.trim(); payload.temporary_password = password; await window.adminUserService.create(payload); }
  ...
  async function renderUsers() {
    ...
    section.querySelectorAll('[data-reset]').forEach(button => button.onclick = async () => { const password = prompt('Nueva contraseña temporal (14+ caracteres, mayúscula, minúscula, número y símbolo):'); if (password === null) return; if (!strongPassword(password)) return notify('La contraseña temporal no cumple la política de seguridad.', true); try { await window.adminUserService.resetPassword(button.dataset.reset, password); await renderUsers(); notify('Contraseña restablecida. Entrégala por un canal seguro.'); } catch (err) { notify(err.message, true); } });
  }
  ...
  function showMandatoryPasswordChange() {
    ...
    document.body.insertAdjacentHTML('beforeend', '<div id="mandatoryPasswordModal" class="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/80"><form id="mandatoryPasswordForm" class="max-w-md w-full bg-white rounded-3xl p-6 space-y-4"><h3 class="font-black text-lg">Actualiza tu contraseña</h3><p class="text-xs text-slate-600">Por seguridad debes cambiar la contraseña temporal antes de continuar.</p><input id="mandatoryPasswordInput" type="password" required autocomplete="new-password" placeholder="14+ caracteres, mayúscula, minúscula, número y símbolo" class="w-full px-3 py-3 border rounded-xl"><div id="mandatoryPasswordError" class="hidden text-xs text-[#de4f5f] font-bold"></div><button class="w-full py-3 rounded-full bg-[#433364] text-white text-sm font-black">Guardar nueva contraseña</button></form></div>');
    document.getElementById('mandatoryPasswordForm').onsubmit = async (event) => { event.preventDefault(); const password = document.getElementById('mandatoryPasswordInput').value; const error = document.getElementById('mandatoryPasswordError'); if (!strongPassword(password)) { error.textContent = 'La contraseña no cumple la política de seguridad.'; error.classList.remove('hidden'); return; } try { await window.adminUserService.changeOwnPassword(password); await window.authService.loadProfile(); document.getElementById('mandatoryPasswordModal').remove(); } catch (err) { error.textContent = err.message; error.classList.remove('hidden'); } };
  }
